...
from bd6_employees join rmanager on rmanager.manager_id =
bd6_employees.id
)
...
i =6;
WHILE i >= 1 LOOP
FOR emp in SELECT id,manager_id,last_name from bd6_employees
where id = i LOOP
...
DECLARE
curs_em CURSOR FOR select * from bd6_employees order by
department_id, salary_in_euro;
...
from bd6_employees b6 join (select b1.id, b2.manager_id from
bd6_employees b1 join bd6_employees b2 on b1.manager_id =
b2.id) b
on b6.id = b.id where b6.manager_id != 1 order by b6.manager_id,
b6.salary_in_euro offset 3)
LOOP
...
CREATE TABLE spiral( f1 integer, f2 integer, f3 integer, f4 integer,
f5 integer );
CREATE OR REPLACE FUNCTION fspiral() RETURNS integer
AS $$
DECLARE
i integer := 1;
n integer := 0;
...
LOOP
CASE mod(i,4)
WHEN 1 THEN INSERT INTO spiral VALUES (n + 1, n + 3, n + 5,
n + 7, n + 9);
WHEN 2 THEN INSERT INTO spiral VALUES (n + 2, n + 4, n + 6,
n + 8, n + 10);
n := n + 10;
WHEN 3 THEN INSERT INTO spiral VALUES (n + 10, n + 8, n +
6, n + 4, n + 2);
WHEN 0 THEN INSERT INTO spiral VALUES (n + 9, n + 7, n + 5,
n + 3, n + 1);
n := n + 10;
END
CASE;
